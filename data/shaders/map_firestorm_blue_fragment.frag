...
// Blue Firestorm parameters
const float FIRE_INTENSITY = 0.6;             // Overall intensity of fire effect
const float ASH_DENSITY = 0.6;                // Density of ash particles
const float HEAT_DISTORTION = 0.4;            // Amount of heat distortion
const vec3 FIRE_COLOR1 = vec3(0.0, 0.5, 1.0); // Blue
const vec3 FIRE_COLOR2 = vec3(0.1, 0.2, 0.8); // Deep blue
const vec3 FIRE_COLOR3 = vec3(0.3, 0.7, 1.0); // Light blue
const float GLOW_STRENGTH = 0.4;              // Strength of the fiery glow
const float WIND_SPEED = 0.4;                 // Speed of wind/fire movement
...
float ashParticle(vec2 uv, float time, float size, float speed) {
    ...
    // Random ash size
    float ashSize = 0.03 + 0.04 * hash(id.x * 33.3 + id.y * 77.7);
...
void main() {
    ...
    vec2 firePos = v_WorldPos * 0.01;
    firePos.y -= time * 1.5; // Fire moves upward
    firePos.x += sin(firePos.y * 4.0) * 0.1; // Swirling flames
    
    float fireNoise = fbm(firePos * 2.0);
    
    // Make fire more intense at the bottom
    float verticalGradient = smoothstep(0.0, 1.0, 1.0 - v_WorldPos.y * 0.001);
    fireNoise *= verticalGradient;
    
    // Add smaller details to fire
    float fireDetails = fbm(firePos * 5.0 + time * 0.5) * 0.5;
    
    // Combine fire effects
    float fireEffect = fireNoise * 0.7 + fireDetails * 0.3;
    fireEffect = smoothstep(0.3, 0.7, fireEffect) * FIRE_INTENSITY;
    
    // Generate ash particles at different scales and speeds
    float ash = 0.0;
    ash += ashParticle(v_WorldPos * 0.03, time, 10.0, 0.2) * 0.5;
    ash += ashParticle(v_WorldPos * 0.05, time * 0.7, 15.0, 0.3) * 0.3;
    ash += ashParticle(v_WorldPos * 0.08, time * 0.5, 20.0, 0.4) * 0.2;
    ...
    // Add ash particles (grey/black with slight blue tint)
    color.rgb = mix(color.rgb, vec3(0.2, 0.22, 0.28), ash * 0.3);
