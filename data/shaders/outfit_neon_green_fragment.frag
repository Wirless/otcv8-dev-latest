...
vec3 getNeonColor(float value) {
    ...
    vec3 yellowGreen = vec3(0.8, 1.0, 0.0);
    
    // Cycle through colors
    value = fract(value);
    
    if (value < 0.25) {
        return mix(darkGreen, brightGreen, value * 4.0);
    } else if (value < 0.5) {
        return mix(brightGreen, limeGreen, (value - 0.25) * 4.0);
    } else if (value < 0.75) {
        return mix(limeGreen, yellowGreen, (value - 0.5) * 4.0);
    } else {
        return mix(yellowGreen, darkGreen, (value - 0.75) * 4.0);
    }
}
