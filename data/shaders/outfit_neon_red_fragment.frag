...
// Red neon colors only
vec3 getNeonColor(float value) {
    // Red neon palette
    vec3 darkRed = vec3(0.5, 0.0, 0.0);
    vec3 brightRed = vec3(1.0, 0.0, 0.0);
    vec3 orange = vec3(1.0, 0.3, 0.0);
    vec3 yellowRed = vec3(1.0, 0.5, 0.0);
    
    // Cycle through colors
    value = fract(value);
    
    if (value < 0.25) {
        return mix(darkRed, brightRed, value * 4.0);
    } else if (value < 0.5) {
        return mix(brightRed, orange, (value - 0.25) * 4.0);
    } else if (value < 0.75) {
        return mix(orange, yellowRed, (value - 0.5) * 4.0);
    } else {
        return mix(yellowRed, darkRed, (value - 0.75) * 4.0);
    }
}
