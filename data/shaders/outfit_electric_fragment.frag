...
// Electric color function
vec3 electricColor(float t) {
    // Electric color palette - blue to white
    vec3 darkBlue = vec3(0.0, 0.2, 1.0);
    ...
    if (t < 0.33) {
        return mix(darkBlue, brightBlue, t * 3.0);
    } else if (t < 0.66) {
        return mix(brightBlue, cyan, (t - 0.33) * 3.0);
    } else {
        return mix(cyan, white, (t - 0.66) * 3.0);
    }
}
