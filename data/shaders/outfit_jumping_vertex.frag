...
void main() {
    ...
    float verticalOffset = jumpCurve * jumpHeight;
    
    // Apply stretching at the peak and squashing at landing
    float stretchAmount = 0.0;
    float squashAmount = 0.0;
    
    // Stretching near the peak (middle of the jump)
    if (jumpPhase > 0.4 && jumpPhase < 0.6) {
        // Calculate stretch amount, peaking at phase 0.5
        float stretchPhase = 1.0 - abs(jumpPhase - 0.5) * 10.0; // 0 to 1 near peak
        stretchAmount = stretchPhase * stretchFactor;
    ...
    if (jumpPhase > 0.9) {
        ...
        squashAmount = squashPhase * squashFactor;
    ...
    // Recombine position with center and apply jump offset
    vec2 finalPos = spriteCenter + offsetFromCenter;
    finalPos.y -= verticalOffset; // Subtract because Y decreases going up in screen coordinates
