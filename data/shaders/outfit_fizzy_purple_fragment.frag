...
// Fizzy Purple parameters
float purpleIntensity = 0.25; // Intensity of purple coloring
...
void main() {
    ...
    // Create purple fizzy soda effect
    // Mix with purple color
    vec3 purpleColor = vec3(0.6, 0.2, 0.9); // Rich purple
    baseColor.rgb = mix(baseColor.rgb, purpleColor, purpleIntensity);
    ...
    baseColor.rgb += vec3(0.9, 0.8, 1.0) * bubbleHighlight * bubbleIntensity * 1.5;
    
    // Add overall fizz glow effect
    float fizzGlow = sin(u_Time * fizzGlowFrequency) * 0.5 + 0.5;
    fizzGlow = fizzGlow * 0.1; // Subtle effect
    
    // Apply glow - stronger at edges like a glass of purple soda
    float edgeGlow = smoothstep(0.0, 0.8, distFromCenter);
    baseColor.rgb += purpleColor * fizzGlow * edgeGlow;
    ...
    baseColor.rgb += vec3(0.9, 0.8, 1.0) * sparkle * 0.5; // Purplish white sparkles
