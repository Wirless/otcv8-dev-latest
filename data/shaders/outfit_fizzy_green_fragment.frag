...
// Fizzy Green parameters
float greenIntensity = 0.25; // Intensity of green coloring
...
void main() {
    ...
    // Create green fizzy soda effect
    // Mix with green color
    vec3 greenColor = vec3(0.1, 0.9, 0.3); // Bright lime green
    baseColor.rgb = mix(baseColor.rgb, greenColor, greenIntensity);
    ...
    baseColor.rgb += vec3(0.9, 1.0, 0.9) * bubbleHighlight * bubbleIntensity * 1.5;
    
    // Add overall fizz glow effect
    float fizzGlow = sin(u_Time * fizzGlowFrequency) * 0.5 + 0.5;
    fizzGlow = fizzGlow * 0.1; // Subtle effect
    
    // Apply glow - stronger at edges like a glass of green soda
    float edgeGlow = smoothstep(0.0, 0.8, distFromCenter);
    baseColor.rgb += greenColor * fizzGlow * edgeGlow;
    ...
    baseColor.rgb += vec3(0.9, 1.0, 0.9) * sparkle * 0.5; // Greenish white sparkles
