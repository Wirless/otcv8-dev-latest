...
// Parameters
float waveAmount = 0.0018;     // Subtle wave for regal gold effect
float pulseFrequency = 0.7;    // Slower pulse for majestic gold feel
float extendRange = 1.2;       // Extend effect beyond texture boundaries
...
void main() {
    ...
    // Enhanced to extend outside texture boundaries
    float waveStrength = waveAmount * pulse * min(distFromCenter * extendRange, 2.0);
    vec2 displacement = vec2(
        cos(angle + u_Time) * waveStrength,
        sin(angle + u_Time) * waveStrength
    );
    
    // Apply additional radial shimmer that extends beyond texture
    float shimmerAmount = 0.002 * pulse;
    displacement += normalize(normalizedPos) * sin(distFromCenter * 10.0 - u_Time * 2.0) * shimmerAmount;
