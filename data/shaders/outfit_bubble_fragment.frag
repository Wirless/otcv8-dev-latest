...
// Bubble parameters
float redBubbleIntensity = 0.25; // Intensity of the red bubble effect
...
void main() {
    ...
    // Create red bubble effect
    // Add overall red tint to the base color
    baseColor.rgb = mix(baseColor.rgb, vec3(1.0, 0.2, 0.2), redBubbleIntensity);
    ...
    vec3 highlightColor = vec3(1.0, 0.5, 0.5); // Light red
    ...
    // Add pop flash color (bright white/yellow)
    vec3 popColor = mix(vec3(1.0, 0.8, 0.8), vec3(1.0, 0.3, 0.2), distFromCenter);
