...
void main() {
    ...
    // Create a 9x9 Gaussian blur kernel (81 samples)
    ...
    float glowAmount = 0.1 + sin(u_Time * 0.5) * 0.05;
    color.rgb += color.rgb * glowAmount;
    
    // Output blurred color with slight color enhancement
    gl_FragColor = color;
    gl_FragColor.rgb *= 1.1; // Slightly enhance colors
} 