...
// Green electric color function
vec3 electricColor(float t) {
    // Green electric color palette
    vec3 darkGreen = vec3(0.0, 0.4, 0.0);
    vec3 brightGreen = vec3(0.0, 0.9, 0.0);
    vec3 limeGreen = vec3(0.4, 1.0, 0.0);
    vec3 yellowGreen = vec3(0.8, 1.0, 0.2);
    
    t = fract(t); // Ensure t is 0 to 1
    
    if (t < 0.33) {
        return mix(darkGreen, brightGreen, t * 3.0);
    } else if (t < 0.66) {
        return mix(brightGreen, limeGreen, (t - 0.33) * 3.0);
    } else {
        return mix(limeGreen, yellowGreen, (t - 0.66) * 3.0);
    }
}
